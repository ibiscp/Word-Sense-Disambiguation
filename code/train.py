...
def parse_args():
    ...
    parser.add_argument("sentence_size", nargs='?', const=100, type=int, default=7, help="The size of the maximum sentence")
...
def keras_model(vocab_size, sentence_size, output_size, mergeMode, lstmLayers, embedding_size, hidden_size=100, dropout=0.2, recurrent_dropout=0.2):
...
if __name__ == '__main__':
    ...
    # Load data
    dataset, t = load_data(dataset_name=args.dataset_name, path=args.resources_path, gold_name=args.gold_name, sentence_size=args.sentence_size)
    train_x = dataset['train_x']
    dev_x = dataset['dev_x']
    train_y = dataset['train_y']
    dev_y = dataset['dev_y']
    vocabulary_size = len([y for y in t.word_index])
    output_size = vocabulary_size #len([y for y in t.word_index if y.startswith('wn:')])
    sentenceSize = args.sentence_size
    ...
    # Train
    grid = gridSearch(build_fn=keras_model, param_grid=param_grid, vocab_size=vocabulary_size, sentence_size=sentenceSize, output_size=output_size)
